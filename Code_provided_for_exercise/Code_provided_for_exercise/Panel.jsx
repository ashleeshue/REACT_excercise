...
export default function Panel({ data, propCart }) {
  ...
  const { cartState, setCartState } = propCart;
    ...
    const updateCart = (priceDifference) => {
        const updatedItems = cntItem > 0 ? [...cartState.items, data.title] : cartState.items.filter(item => item !== data.title);
    ...
    return (
        <div className='card'>
            <img src={data.img} width="80px" height="80px" alt={data.title} />
            <h3>{data.title}</h3>
